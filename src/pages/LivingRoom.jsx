...
//       <h1 className="text-3xl font-bold mb-6">Living Room Products</h1>
...
const LivingRoom = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-4 mb-4">
          {["all", "price", "color", "material"].map((type) => (
            <button
              key={type}
              className={`px-4 py-2 rounded ${
                filterType === type ? "bg-yellow-500 text-black" : "bg-gray-300"
              }`}
              onClick={() => {
                setFilterType(type);
                setFilterValue("");
          ...
          <div className="flex flex-wrap gap-4">
            {["low", "mid", "high"].map((price) => (
              <button
                ...
                onClick={() => setFilterValue(price)}
                aria-pressed={filterValue === price}
              >
                {price === "low"
                  ? "Low (< $200)"
                  : price === "mid"
          ...
          <div className="flex flex-wrap gap-4">
            {["black", "white", "grey", "brown", "tan", "blue"].map((color) => (
              <button
                ...
                onClick={() => setFilterValue(color)}
                aria-pressed={filterValue === color}
              >
                {color}
              </button>
            ))}
          ...
          <div className="flex flex-wrap gap-4">
            {["leather", "linen", "microfiber"].map((material) => (
              <button
                ...
                onClick={() => setFilterValue(material)}
                aria-pressed={filterValue === material}
              >
                {material}
              </button>
            ))}
